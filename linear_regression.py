"""
Python file that contains the code that performs linear regression in the 'master' computer.
...
def master(
    ...
    if if_verbose:
        ...
        URL_INIT = "http://asia-south1-phonic-command-xxxxxx.cloudfunctions.net/function-init?dataset_name={}&no_of_iter={}&no_of_nodes={}&choice={}&if_ones={}&c={}&factor={}&variety={}&d={}".format(
            DATASET_NAME, 0, NO_OF_NODES, CHOICE, IF_ONES, c, FACTOR, "linear", red
        )
        time1 = time.time()
    ...
    while iterno < NO_OF_ITER:
        ...
        if iterno == 0:
            cost_transition.append(
                np.sum(np.square(np.dot(X, wt) - np.reshape(Y, (-1, 1))))
                / np.shape(X)[0]
            )
            test_cost_transition.append(
                np.sum(np.square(np.dot(Xtest, wt) - np.reshape(Ytest, (-1, 1))))
                / np.shape(Xtest)[0]
            )
        wt_new = wt
        ...
        cost = (
            np.sum(np.square(np.dot(X, wt_new) - np.reshape(Y, (-1, 1))))
            / np.shape(X)[0]
        )
        ...
        cost = (
            np.sum(np.square(np.dot(Xtest, wt_new) - np.reshape(Ytest, (-1, 1))))
            / np.shape(Xtest)[0]
        )
    ...
    np.savetxt(
        "./{}/test_cost_transition.csv".format(folder_name),
        np.array(test_cost_transition),
        delimiter=",",
    )
