"""
Python file that contains the code that performs logistic regression in the 'master' computer.
'function-init'                 Name of the 'initiator' Google Cloud Function
'function-n<integer>'           Name of 'node' Google Cloud Functions

Note: Replace the URLs with your appropriate Google Cloud Function HTTP Trigger URLs once you deploy 
the 'initiator' and 'node' Google Cloud Functions. Both 'initiator' and 'node' Google Cloud Functions
need to be 'HTTP Triggered'.
...
def plot_confusion_matrix(y_true, y_pred, iterno, folder_name, choice, if_labels=False):
    ...
    if choice == "train":
        print(
            "\nTrain --- Iter No:{0}   Accuracy: {1:.5f}%".format(
                iterno + 1, 100 * np.trace(cf_mat) / np.sum(np.sum(cf_mat))
            )
        )
        acc_transition.append(100 * np.trace(cf_mat) / np.sum(np.sum(cf_mat)))
    elif choice == "test":
        print(
            "\nTest ---- Iter No:{0}   Accuracy: {1:.5f}%".format(
                iterno + 1, 100 * np.trace(cf_mat) / np.sum(np.sum(cf_mat))
            )
        )
    ...
    plt.savefig(
        "./{}/{}_conf_mat_iter_{}.png".format(folder_name, choice, iterno + 1),
        bbox_inches="tight",
    )
    # plt.show()
    plt.close()
...
def master(
    ...
    folder_name = "task-{}-data-{}-test-{}-nod-{}-it-{}-choice-{}-c-{}-alg-{}-d-{}-ivl-{}-ident-{}".format(
        "log",
        DATASET_NAME.strip(".zip"),
        TEST_DATASET.strip(".zip"),
        NO_OF_NODES,
        NO_OF_ITER,
        CHOICE,
        c,
        ...
        red,
        if_variable_lrate,
        np.random.randint(0, high=10000),
    )
    print(
    ...
    blobber(
        TEST_DATASET, "./{}/test_dataset.zip".format(folder_name), bucket1, "download"
    )
    data_ = Misc.load_dataset("./{}/test_dataset.zip".format(folder_name), IF_ONES)[
        "data"
    ]
    Xtest = data_[:, :-1]
    Ytest = data_[:, -1]
    ...
    if if_verbose:
        ...
        URL_INIT = "http://asia-south1-phonic-command-xxxxxx.cloudfunctions.net/function-init?dataset_name={}&no_of_iter={}&no_of_nodes={}&choice={}&if_ones={}&c={}&factor={}&variety={}&d={}".format(
            DATASET_NAME, 0, NO_OF_NODES, CHOICE, IF_ONES, c, FACTOR, VARIETY, red
        )
        time1 = time.time()
    ...
    if if_verbose:
        if if_custom_init_wt:
            try:
                blobber(
                    "weights_iter{}.csv".format(0), "initial_wt.csv", bucket1, "upload"
                )
                print(
                    "\n$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n"
                )
                print(
                    '---- "initial_wt.csv" found. Proceeding with algorithm using custom initial weights.'
                )
                logit(
                    "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
                )
                ...
                print(
                    '---- "initial_wt.csv" not found. Proceeding with algorithm using randomly initialized weights.'
                )
                logit(
                    "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"
                )
    ...
    blobber(
        "assgn_matrix.csv",
        "./{}/assgn_matrix.csv".format(folder_name),
        bucket2,
        "download",
    )
    ...
    while iterno < NO_OF_ITER:
        ...
        urls = [
            "https://asia-south1-phonic-command-xxxxxx.cloudfunctions.net/function-n{}?dataset_name=dataset_{}.csv&iter_no={}&task={}&n_labels={}".format(
                (i + 51), (i + 1), iterno, VARIETY, no_of_unique_labels
            )
        ...
        if if_verbose:
            with concurrent.futures.ThreadPoolExecutor(
                ...
                for future in concurrent.futures.as_completed(future_to_url):
                    ...
                    try:
                        data = future.result()
                        print(
                            '---- The outcome of "{0}" is "{1}". \nCompleted in {2:.5f}s.\n'.format(
                                temp1, data, timex - time_start_dict[temp1]
                            )
                        )
                        logit(
                            '---- The outcome of "%s" is "%s". Completed in %ss.'
                            % (temp1, data, timex - time_start_dict[temp1])
                        )
                        ...
                        logit(
                            '---- Error encountered in "%s"" : "%s". Completed in %ss.'
                            % (temp1, data, timex - time_start_dict[temp1])
                        )
                        time_dict[temp1] = timex - time_start_dict[temp1]
            time_dict = {
                k: v for k, v in sorted(time_dict.items(), key=lambda item: item[1])
            }
            http_time = http_time + list(time_dict.values())[-1]
            print(
                "#####   Total time taken for HTTP calls: {0:.5f}s   #####\n".format(
                    list(time_dict.values())[-1]
                )
            )
            print("---- Dict of time taken for the nodes:\n", time_dict)
            logit(
                "#####   Total time taken for HTTP calls: %ss   #####"
                % (list(time_dict.values())[-1])
            )
            ...
            http_time = http_time + list(time_dict.values())[-1]
            print(
                "#####   Total time taken for HTTP calls: {0:.5f}s   #####".format(
                    list(time_dict.values())[-1]
                )
            )
            logit(
        ...
        all_time_data_dict["http_iter_{}_w".format(iterno + 1)] = list(
            time_dict.values()
        )[-1]
        # Saving the time_dict for this iteration
        with open(
            "./{}/time_dict_iterno_{}.csv".format(folder_name, iterno + 1), "w"
        ) as csv_file:
        ...
        if algo == "time_strag":
            ...
            print(
                "---- We will be using gradients from ({}/{}) nodes in this iteration.".format(
                    len(valid_nodes), NO_OF_NODES
                )
            )
            logit(
                "---- We will be using gradients from (%s/%s) nodes in this iteration."
                % (len(valid_nodes), NO_OF_NODES)
            )
            nodes_num_used.append(float(len(valid_nodes)))
            ...
            sys.exit()
        all_time_data_dict["http_iter_{}_wout".format(iterno + 1)] = time_dict[
            valid_nodes[-1]
        ]
        # Saving the valid nodes for this iteration into csv
        with open(
            "./{}/valid_nodes_iterno_{}.csv".format(folder_name, iterno + 1), "w"
        ) as f:
            write = csv.writer(f)
        ...
        print(
            "\n---- Time spent on nodes(excluding stragglers) in this iteration: {0:.5f}s".format(
                time_dict[valid_nodes[-1]]
            )
        )
        print(
            "---- The nodes whose gradient outputs will be used for the weight update:\n {}\n".format(
                valid_nodes
            )
        )
        logit(
            "---- Time spent on nodes(excluding stragglers) in this iteration: %ss"
            % (time_dict[valid_nodes[-1]])
        )
        logit(
            "---- The nodes whose gradient outputs will be used for the weight update: %s"
            % (valid_nodes)
        )
        ...
        grads = storage_client.list_blobs(
            "data_tum_node-to-master", prefix="grad_iterno_{}_".format(iterno)
        )
        ...
        if if_verbose:
            ...
            logit(
                "---- We use the following gradients to update the weights: %s"
                % (grad_downloads)
            )
        else:
            for x in grads:
        ...
        if iterno == 0:
            if VARIETY == "logistic":
                ...
                cost_transition.append(
                    np.sum(
                        -np.multiply(np.log10(Yhat), np.reshape(Y, (-1, 1)))
                        - np.multiply(np.log10(1 - Yhat), 1 - np.reshape(Y, (-1, 1)))
                    )
                    / np.shape(X)[0]
                )
                Y_prediction = np.round(Yhat)
                conf_mat_transition.append(
                    plot_confusion_matrix(Y, Y_prediction, -1, folder_name, "train")
                )
                Y_test_prediction = np.round(sigmoid(np.dot(Xtest, wt)))
                test_conf_mat_transition.append(
                    plot_confusion_matrix(
                        Ytest, Y_test_prediction, -1, folder_name, "test"
                    )
                )
            ...
            if algo == "StocGD":
                if if_variable_lrate:
                    wt_new = (
                        ...
                        - (
                            LRATE
                            * compute_alpha(iterno + 1)
                            / (red * (1 - THRESHOLD / NO_OF_NODES))
                        )
                        * grad
                    )
                else:
                    wt_new = (
                        wt_new - (LRATE / (red * (1 - THRESHOLD / NO_OF_NODES))) * grad
                    )
            else:
                if if_variable_lrate:
        ...
        if VARIETY == "logistic":
            ...
            conf_mat_transition.append(
                plot_confusion_matrix(Y, Y_prediction, iterno, folder_name, "train")
            )
            Y_test_prediction = np.round(sigmoid(np.dot(Xtest, wt_new)))
            test_conf_mat_transition.append(
                plot_confusion_matrix(
                    Ytest, Y_test_prediction, iterno, folder_name, "test"
                )
            )
        elif VARIETY == "multi-logistic":
        ...
        blobber(
            "weights_iter{}.csv".format(iterno + 1), "wt_new.csv", bucket1, "upload"
        )
    ...
    if if_custom_init_wt:
        os.replace(
            os.path.join(cwd, "initial_wt.csv"), os.path.join(path, "initial_wt.csv")
        )
    if algo == "time_strag":
        np.savetxt(
            "./{}/time_strag-num_nodes_used.csv".format(folder_name),
            np.array(nodes_num_used),
            delimiter=",",
        )
    ...
    for i in range(NO_OF_ITER + 1):
        np.savetxt(
            "./{}/conf_mat_iter_{}.csv".format(folder_name, i),
            np.array(conf_mat_transition[i]),
            delimiter=",",
        )
    for i in range(NO_OF_ITER + 1):
        np.savetxt(
            "./{}/test_conf_mat_iter_{}.csv".format(folder_name, i),
            np.array(test_conf_mat_transition[i]),
            delimiter=",",
        )
    ...
    for i in range(NO_OF_ITER + 1):
        np.savetxt(
            "./{}/wt_iter_{}.csv".format(folder_name, i),
            wt_transition[i],
            delimiter=",",
        )
    np.savetxt(
        "./{}/cost_transition.csv".format(folder_name),
        np.array(cost_transition),
        delimiter=",",
    )
    np.savetxt(
        "./{}/acc_transition.csv".format(folder_name),
        np.array(acc_transition),
        delimiter=",",
    )
    np.savetxt(
        "./{}/test_acc_transition.csv".format(folder_name),
        np.array(test_acc_transition),
        delimiter=",",
    )
    ...
    print(
        "\tTotal time taken for initiator and nodes(excluding stragglers): {0:.5f}s".format(
            sum_time
        )
    )
    print("\tTotal time taken for initiator and nodes: {0:.5f}s".format(http_time))
    print(
        "\tTime saved by excluding stragglers: {0:.5f}s ({1:.3f} percent of {2:.5f})".format(
            http_time - sum_time, 100 * (http_time - sum_time) / http_time, http_time
        )
    )
    if algo == "time_strag":
        print(
            "\tNumber of nodes used for weight updates on average: ({}/{})".format(
                sum(nodes_num_used) / len(nodes_num_used), NO_OF_NODES
            )
        )
    ...
    logit(
        "Total time taken for initiator and nodes(excluding stragglers): %ss"
        % (sum_time)
    )
    logit("Total time taken for initiator and nodes: %ss" % (http_time))
    logit(
        "Time saved by excluding stragglers: %ss (%s percent of %s)"
        % (http_time - sum_time, 100 * (http_time - sum_time) / http_time, http_time)
    )
    if algo == "time_strag":
        logit(
            "\tNumber of nodes used for weight updates on average: (%s/%s)"
            % (sum(nodes_num_used) / len(nodes_num_used), NO_OF_NODES)
        )
