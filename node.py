"""
This is the code that needs to be used in each Google Cloud Function that functions as a 'node'
in this distributed computing setup.
"""
import os
...
def node(request):
    ...
    if (
        ...
        and "iter_no" in request.args
        and "task" in request.args
        and "n_labels" in request.args
    ):
        ...
        and "iter_no" in request_json
        and "task" in request_json
        and "n_labels" in request_json
    ):
        ...
        return "Appropriate parameters not given. 'dataset_name', 'task', 'if_ones',  and 'iter_no' need to be given."
    ##############################################################
    ...
    blobber(
        "weights_iter{}.csv".format(ITER_NO), "/tmp/weights.csv", bucket1, "download"
    )
    ...
    if TASK == "linear":
        ...
        grad = (
            np.transpose(X)
            @ (sigmoid(np.dot(X, wt)) - np.reshape(y, (-1, 1)))
            / np.shape(mat)[1]
        )
        np.savetxt("/tmp/grad.csv", grad, delimiter=",")
